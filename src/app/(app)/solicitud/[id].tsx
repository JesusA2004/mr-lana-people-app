...
export default function SolicitudDetalleScreen() {
  ...
  const allDetails: { icon: keyof typeof Ionicons.glyphMap; label: string; value?: string }[] = [
    { icon: 'chatbox-ellipses-outline', label: 'Motivo', value: pickString(solicitud, ['motivo']) },
    { icon: 'reader-outline', label: 'Observaciones', value: pickString(solicitud, ['observaciones']) },
    { icon: 'calendar-outline', label: 'Fecha de inicio', value: pickString(solicitud, ['fecha_inicio']) },
    { icon: 'calendar-outline', label: 'Fecha de fin', value: pickString(solicitud, ['fecha_fin']) },
    { icon: 'alert-circle-outline', label: 'Motivo de rechazo', value: pickString(solicitud, ['motivo_rechazo']) },
  ];
                  ...
                  <View key={item.label} style={[styles.detailRow, index === details.length - 1 && styles.detailRowLast]}>
                    ...
                    <View style={styles.detailText}>
                      ...
                      <Text style={styles.detailValue}>
                        {item.label.startsWith('Fecha') && item.value ? formatDateLong(item.value) : item.value}
                      </Text>
                    </View>
                  </View>
                ))}
...
const styles = StyleSheet.create({
  ...
  type: { fontSize: FontSize.xl, fontWeight: '800', color: Colors.text, flexShrink: 1 },
  folio: { fontSize: FontSize.sm, color: Colors.textMuted, fontWeight: '600' },
